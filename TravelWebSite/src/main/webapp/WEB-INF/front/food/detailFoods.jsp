...
$(function(){
	if("${loginSession}" !=null){
		// 判断是否收藏
		$.ajax({
			url : '${ctx}/foods/isfavoriteFood.do',
			type : 'POST',
			data : {
				foodsId:$("#foodId").val(),
				userId:$("#userId").val()
...
function addToFavorite(aObj,foodsId){
	...
	if(flag){
		// 添加收藏
		$.ajax({
			url : '${ctx}/foods/addfavoriteFood.do',
			type : 'POST',
			data : {
				userId:userId,
				foodsId:foodsId
		...
		// 删除收藏
		$.ajax({
			url : '${ctx}/foods/delfavoriteFood.do',
			type : 'POST',
			data : {
				userId:userId,
				foodsId:foodsId
...
function to_comment(){
	...
	if(confirm("确认提交评论吗?")){
		$.post(
			"${ctx}/foods/toCommentFoods.do",
			{"userId":userId,"foodsId":foodsId,"content":content},
			function(data){
				if (data == "success") {
					alert("评论成功！");
...
<body>
	<div class="container">
		<div class="row clearfix">
			<ul class="hd-nav fl">
				<li><a href="#part1">美食详细</a>
				<li><a href="#part2">美食订购</a></li>
			...
			<div class="col-md-12 column "
				...
				<form id="queryCommentsListForm" class="navbar-form navbar-left" role="search"
					action="${ctx}/foods/detailFoodsUI.do" method="post">
					...
					<input type="hidden" name="totalPages" id="totalPages"
									 ...
									 type="hidden" id="id" name="id" value="${food.id }">
	...
	<div class="title" id="part1">美食详细</div>
	<div class="content">
		...
		<div class="persional_property">
			<div class="person_info_con">
				...
				<dl class="person-photo" style="display: inline-block; float: left;">
					<dt>
						<a href=""><img src="${ctx}${food.url}" class="header" style="width:300px;height:300px"><span
				...
				<dl class="person-info"
					...
					<input type="hidden" id="userId" name="userId" value="${loginSession.id}" />
					<dd class="person-detail" style="margin-top: 5px">
					...
					<dd class="person-detail" style="margin-top: 5px">
						<span class="info_null">热度：</span><span style="color: #eb7350;">
							<b>${food.commentTimes }</b>
						</span>
					</dd>
					<dd class="person-detail" style="margin-top: 5px">
						<span class="info_null">产地：</span><span style="color: #eb7350;">
							<b>${food.addressName }</b>
						</span>
					</dd>
					<dd class="person-detail" style="margin-top: 5px">
						<span class="info_null">价格：</span><span style="color: #eb7350;">
							<b>${food.price }</b>
						</span>
					</dd>
					...
					<dd class="person-detail" style="margin-top: 5px">
						<span class="info-null"><a href="javascript:void(0);" class="" onclick="addToFavorite(this,'${food.id }')"  id="J_favorite">
					...
					<dd class="person-detail" style="margin-top: 5px">
						<span class="info-null">添加评论：</span><span style="color: #eb7350;">
							<form class="form" id="form" action="${ctx}/foods/toComment.do" method="post">
				<table>
                       <tr><td colspan="1"><input type="text" name="content" id="toContent"  style= " margin-left: -170px; width: 400px; height: 50px;"/></td></tr>
                       <input type="hidden" name="foodsId" id="foodsId" value="${food.id }"/>
	...
	<div class="title" id="part2">美食评价</div>
